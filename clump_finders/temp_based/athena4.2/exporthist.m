...
getAllClumpSizes[file_] := 
  Module[{data = readtemp[file], t, allsizes, 
    tfloor},(*first threshold the data*)
   tfloor = Min[data];
...
makeClumpHistograms[dir_] := 
  Module[{fnames = Drop[FileNames[dir <> "*.vtk"], 1], clumpdata, 
    tlim, segments}, clumpdata = Map[getAllClumpSizes, fnames];
...
myfmt2[num_] := 
  ScientificForm[num, {10, 4}, NumberPadding -> {" ", "0"}, 
   NumberFormat -> (#1 <> "e" <> #3 &)];

myfmt[num_] := 
  ToString[If[num != 0 && Abs[Log[10, num]] >= 3, myfmt2[num], 
    myfmt1[num]]];
